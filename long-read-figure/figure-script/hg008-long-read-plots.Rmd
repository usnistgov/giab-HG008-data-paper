---
title: "HG008 long read data plot"
...
library(ggpubr)
```
Long read data summary plots for figure 5 in the HG008 Scientific Data manuscript. Documentation of data (get_bam_stat_output and extract_samtools_stats_output) used in these plots can be found in "long read processing for HG008 data paper figures.ipynb". Long read datasets included in the HG008 scientific data paper are:
...
read_len_fig <- aln_stat_df %>%
  sample_frac(0.10) %>%
  ggplot(aes(weight = aln_lengthmax, 
             x = aln_lengthmax, 
             color = data_id, 
             linetype = sample, 
             fill = data_id)) + 
  geom_density(alpha = 0.5, linewidth=1) +  # Use geom_density for standard density plots
  ...
  theme_pubr() +
  theme_bw() + #black and white background to plot
  theme(axis.line.y.left = element_blank(), # Remove y-axis line on the left
        ...
        panel.border = element_blank()) # Adjust size for better readability,panel args are for theme_grey to remove lines from background
...
cov_dist_fig <- genome_cov_df %>% 
  ...
  theme(strip.text.y = element_text(size = 7, 
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
        panel.border = element_blank()) # Adjust size for better readability
...
cov_v_length_fig <- ggplot(cov_df) + 
    geom_path(aes(x = length_bins, y = cum_coverage, 
                  color = data_id, 
                  linetype = sample), 
              size=1) +
    scale_color_brewer(palette="Paired") +
  ...
  theme(strip.text.y = element_text(size = 9),
        ...
        legend.direction  ="vertical", panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank()) #needed for legend in panel
...
ggsave("Figure 5.pdf", width = 12, height = 8, units = "in")
...
```{r cov_uL_100kb}
cov_100kb_UL1 <- cov_df %>% filter(data_id == "ONT-UL-1_T") %>% filter(length_bins == 100000)
cov_100kb_UL2 <- cov_df %>% filter(data_id == "ONT-UL-2_T") %>% filter(length_bins == 100000)
```